...
public class NumberUtil {
	...
	private static String [] ms_digitsTR = {"", "bin", "milyon", "milyar"};
	...
	private static String numToTextMax3DigitsTR(int val)
	{
		...
		if (a != 0) {
			if (a != 1)
				str += String.format("%s ",ms_onesTR[a]);
			str += "yüz ";
		}

		if (b != 0)
			str += String.format("%s ",ms_tensTR[b]);


		if (c != 0)
			str += String.format("%s ",ms_onesTR[c]);
	...
	public static String numberToText(int val) {
		int [] threeDigits = getDigits(3,val);

		String str = "";

		if (threeDigits.length == 1)
			return numToTextMax3DigitsTR(val);


		for (int i = 0; i < threeDigits.length; ++i) {
			int threeDigitsVal = threeDigits[i];

			if (threeDigitsVal == 0)
				continue;


			str += String.format("%s %s ",numToTextMax3DigitsTR(threeDigitsVal), ms_digitsTR[threeDigits.length - 1 - i]);
